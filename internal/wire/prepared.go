...
// asks it.
//
// It is read-only after NewPrepared returns and safe for concurrent use.
type Prepared struct {
	...
	Questions []byte
	// Entries lists the questions in the order they were added.
...
// is questions and whose tables are entries. The names in entries must be
// unique: the root package rejects a repeated name before it gets here. The
// slices are kept, not copied.
func NewPrepared(questions []byte, entries []PreparedQuestion) *Prepared {
...
// returned pointer refers into p.Entries.
