...
func TestPreparedForRejections(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"error: (1) duplicate wire name": {
			...
			wantMsg: `PreparedFor[engine.rejDupName]: field Other: Question "Spam" is added more than once; question names must be unique. Field Spam asks it first.`,
		},
		"error: (2) duplicate option label": {
			prepare: PreparedFor[rejDupOption],
			wantMsg: `PreparedFor[engine.rejDupOption]: field Tone: Choice question "Tone" has option "calm" more than once; option labels must be unique.`,
		},
		"error: (3) duplicate level text": {
			prepare: PreparedFor[rejDupLevel],
			wantMsg: `PreparedFor[engine.rejDupLevel]: field Urgency: Score question "Urgency" has level "low" more than once; levels must be unique.`,
		},
		"error: (4) score without levels": {
			prepare: PreparedFor[rejNoLevels],
			wantMsg: `PreparedFor[engine.rejNoLevels]: field Urgency: Score question "Urgency" has no criteria; at least one score is required. List the levels, lowest first, as levels=low|high.`,
		},
		"error: (4) choice without options": {
			prepare: PreparedFor[rejNoOptions],
			wantMsg: `PreparedFor[engine.rejNoOptions]: field Tone: Choice question "Tone" has no options; list them, each optionally described, as options=calm=polite|angry.`,
		},
		"error: (5) missing kind": {
			prepare: PreparedFor[rejNoKind],
			wantMsg: `PreparedFor[engine.rejNoKind]: field Spam: the typesafe tag has no kind; add kind=noul.`,
		},
		"error: (5) missing kind: answer field without a tag": {
			prepare: PreparedFor[rejUntagged],
			wantMsg: `PreparedFor[engine.rejUntagged]: field Spam: the NoulAnswer field has no typesafe tag, so no kind; every answer field asks a question: add a tag such as typesafe:"kind=noul".`,
		},
		"error: (6) unknown kind": {
			prepare: PreparedFor[rejUnknownKind],
			wantMsg: `PreparedFor[engine.rejUnknownKind]: field Spam: unknown kind "yesno"; the kinds are noul, choice and score.`,
		},
		"error: (7) kind/field-type mismatch": {
			prepare: PreparedFor[rejKindMismatch],
			wantMsg: `PreparedFor[engine.rejKindMismatch]: field Tone: kind=choice needs a ChoiceAnswer field, and the field is a NoulAnswer; make the kind and the field's type agree.`,
		},
		"error: (7) kind/field-type mismatch: tag on a string field": {
			prepare: PreparedFor[rejTaggedString],
			wantMsg: `PreparedFor[engine.rejTaggedString]: field Note: a typesafe tag needs a NoulAnswer, ChoiceAnswer or ScoreAnswer field, and the field is a string.`,
		},
		"error: (7) kind/field-type mismatch: tag on a field of an unnamed struct type": {
			prepare: PreparedFor[rejTaggedUnnamedStruct],
			wantMsg: `PreparedFor[engine.rejTaggedUnnamedStruct]: field Meta: a typesafe tag needs a NoulAnswer, ChoiceAnswer or ScoreAnswer field, and the field is a struct {...}.`,
		},
		"error: (7) kind/field-type mismatch: self-referential pointer": {
			prepare: PreparedFor[rejSelfPointer],
			wantMsg: `PreparedFor[engine.rejSelfPointer]: field Spam: a typesafe tag needs a NoulAnswer, ChoiceAnswer or ScoreAnswer field, and the field is a engine.selfPointer.`,
		},
		"error: (8) pointer field": {
			prepare: PreparedFor[rejPointer],
			wantMsg: `PreparedFor[engine.rejPointer]: field Spam: the field is a pointer, *engine.NoulAnswer; answer fields are values: make it a NoulAnswer, with optional in its tag if the answer may be absent.`,
		},
		"error: (8) pointer field: untagged": {
			prepare: PreparedFor[rejUntaggedPointer],
			wantMsg: `PreparedFor[engine.rejUntaggedPointer]: field Spam: the field is a pointer, **engine.NoulAnswer; answer fields are values: make it a NoulAnswer, with optional in its tag if the answer may be absent.`,
		},
		"error: (9) tagged field inside an embedded struct": {
			prepare: PreparedFor[rejEmbeddedTagged],
			wantMsg: `PreparedFor[engine.rejEmbeddedTagged]: field TaggedBase: fields of an embedded struct are not promoted, and TaggedBase.Spam has a typesafe tag; declare Spam in engine.rejEmbeddedTagged itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field inside an embedded pointer to a struct": {
			prepare: PreparedFor[rejEmbeddedPointer],
			wantMsg: `PreparedFor[engine.rejEmbeddedPointer]: field TaggedBase: fields of an embedded struct are not promoted, and TaggedBase.Spam has a typesafe tag; declare Spam in engine.rejEmbeddedPointer itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field two embeddings deep": {
			prepare: PreparedFor[rejEmbeddedDeep],
			wantMsg: `PreparedFor[engine.rejEmbeddedDeep]: field OuterBase: fields of an embedded struct are not promoted, and OuterBase.TaggedBase.Spam has a typesafe tag; declare Spam in engine.rejEmbeddedDeep itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field inside an unexported embedded struct": {
			prepare: PreparedFor[rejEmbeddedUnexported],
			wantMsg: `PreparedFor[engine.rejEmbeddedUnexported]: field taggedBase: fields of an embedded struct are not promoted, and taggedBase.Spam has a typesafe tag; declare Spam in engine.rejEmbeddedUnexported itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field inside a self-embedding struct": {
			prepare: PreparedFor[rejEmbeddedCyclic],
			wantMsg: `PreparedFor[engine.rejEmbeddedCyclic]: field CyclicTagged: fields of an embedded struct are not promoted, and CyclicTagged.Spam has a typesafe tag; declare Spam in engine.rejEmbeddedCyclic itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) unexported tagged field inside an embedded struct": {
			prepare: PreparedFor[rejEmbeddedHiddenTag],
			wantMsg: `PreparedFor[engine.rejEmbeddedHiddenTag]: field hiddenTaggedBase: fields of an embedded struct are not promoted, and hiddenTaggedBase.spam has a typesafe tag; declare spam in engine.rejEmbeddedHiddenTag itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) unexported tagged field inside a named struct field": {
			prepare: PreparedFor[rejNamedHiddenTag],
			wantMsg: `PreparedFor[engine.rejNamedHiddenTag]: field Inner: fields of a nested struct are not read, and Inner.spam has a typesafe tag; declare spam in engine.rejNamedHiddenTag itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field inside a named struct field": {
			prepare: PreparedFor[rejNamedStruct],
			wantMsg: `PreparedFor[engine.rejNamedStruct]: field Inner: fields of a nested struct are not read, and Inner.Spam has a typesafe tag; declare Spam in engine.rejNamedStruct itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field inside a named pointer-to-struct field": {
			prepare: PreparedFor[rejNamedPointer],
			wantMsg: `PreparedFor[engine.rejNamedPointer]: field Inner: fields of a nested struct are not read, and Inner.Spam has a typesafe tag; declare Spam in engine.rejNamedPointer itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field deep inside a named field": {
			prepare: PreparedFor[rejNamedDeep],
			wantMsg: `PreparedFor[engine.rejNamedDeep]: field Wrap: fields of a nested struct are not read, and Wrap.TaggedBase.Spam has a typesafe tag; declare Spam in engine.rejNamedDeep itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field behind two named fields": {
			prepare: PreparedFor[rejNamedTwoLevels],
			wantMsg: `PreparedFor[engine.rejNamedTwoLevels]: field Outer: fields of a nested struct are not read, and Outer.Inner.Spam has a typesafe tag; declare Spam in engine.rejNamedTwoLevels itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field behind a named field of an embedded struct": {
			prepare: PreparedFor[rejEmbeddedNamedInner],
			wantMsg: `PreparedFor[engine.rejEmbeddedNamedInner]: field NamedOuter: fields of an embedded struct are not promoted, and NamedOuter.Inner.Spam has a typesafe tag; declare Spam in engine.rejEmbeddedNamedInner itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) named field of a typed question set type": {
			prepare: PreparedFor[rejNamedTicket],
			wantMsg: `PreparedFor[engine.rejNamedTicket]: field Prev: fields of a nested struct are not read, and Prev.Billing has a typesafe tag; declare Billing in engine.rejNamedTicket itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field inside an unexported named field": {
			prepare: PreparedFor[rejNamedUnexported],
			wantMsg: `PreparedFor[engine.rejNamedUnexported]: field inner: fields of a nested struct are not read, and inner.Spam has a typesafe tag; declare Spam in engine.rejNamedUnexported itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) tagged field inside a field of an unnamed struct type": {
			prepare: PreparedFor[rejNamedAnonymousType],
			wantMsg: `PreparedFor[engine.rejNamedAnonymousType]: field Meta: fields of a nested struct are not read, and Meta.Spam has a typesafe tag; declare Spam in engine.rejNamedAnonymousType itself, since PreparedFor reads only the struct's own fields.`,
		},
		"error: (9) unexported field with a tag": {
			prepare: PreparedFor[rejUnexported],
			wantMsg: `PreparedFor[engine.rejUnexported]: field spam: the field is unexported and has a typesafe tag; only exported fields are answered: export the field or remove the tag.`,
		},
		...
		"error: (10) non-struct T: pointer to a struct": {
			...
			wantMsg: `PreparedFor[*engine.Ticket]: *engine.Ticket is a pointer; pass the struct type itself: PreparedFor[engine.Ticket].`,
		},
		...
		"error: (11) reserved name": {
			...
			wantMsg: `PreparedFor[engine.rejReserved]: field Model: the question name "model" is reserved (type, model, usage and answers are); give the field another name with name=.`,
		},
		"error: (11) reserved name after an ignored field": {
			prepare: PreparedFor[rejReservedAfterIgnored],
			wantMsg: `PreparedFor[engine.rejReservedAfterIgnored]: field Usage: the question name "usage" is reserved (type, model, usage and answers are); give the field another name with name=.`,
		},
		"error: (12) optional on a non-answer field": {
			prepare: PreparedFor[rejOptionalString],
			wantMsg: `PreparedFor[engine.rejOptionalString]: field Note: optional applies only to NoulAnswer, ChoiceAnswer and ScoreAnswer fields, and the field is a string.`,
		},
		"error: (13) tag syntax: unknown key": {
			prepare: PreparedFor[rejUnknownKey],
			wantMsg: `PreparedFor[engine.rejUnknownKey]: field Spam: typesafe tag: unknown key "weight"; the keys are kind, name, instructions, yes, no, options, levels and optional.`,
		},
		"error: (13) tag syntax: unterminated escape": {
			prepare: PreparedFor[rejUnterminated],
			wantMsg: `PreparedFor[engine.rejUnterminated]: field Spam: typesafe tag: unterminated escape: a "\" at the end of the tag; write "\\" for a backslash.`,
		},
		"error: (13) tag syntax: key the kind does not take": {
			prepare: PreparedFor[rejKeyOutsideKind],
			wantMsg: `PreparedFor[engine.rejKeyOutsideKind]: field Spam: typesafe tag: options does not apply to kind=noul; a noul takes kind, name, instructions, yes, no and optional.`,
		},
		"error: (14) no answer fields": {
			prepare: PreparedFor[rejEmpty],
			wantMsg: `PreparedFor[engine.rejEmpty]: At least one question is required. engine.rejEmpty has no answer fields: give it one NoulAnswer, ChoiceAnswer or ScoreAnswer field per question, each with a typesafe tag.`,
		},
	}
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
...
func TestPreparedForIgnoredFields(t *testing.T) {
	...
	want5 := `PreparedFor[engine.ignoredFields]: field Second: the ScoreAnswer field has no typesafe tag, so no kind; every answer field asks a question: add a tag such as typesafe:"kind=score".`
...
func TestPreparedForTypeIdentity(t *testing.T) {
	tests := map[string]struct {
		...
		"error: (7) a type defined from an answer type is not one": {
			...
			wantMsg: `PreparedFor[engine.definedTagged]: field Spam: a typesafe tag needs a NoulAnswer, ChoiceAnswer or ScoreAnswer field, and the field is a engine.definedNoul.`,
		},
		...
		"error: (5) a generic struct whose type argument makes an untagged answer field": {
			...
			wantMsg: `PreparedFor[engine.genericSet[github.com/zchee/typesafe-sdk-go/internal/engine.NoulAnswer]]: field Extra: the NoulAnswer field has no typesafe tag, so no kind; every answer field asks a question: add a tag such as typesafe:"kind=noul".`,
		},
