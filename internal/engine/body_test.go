...
func TestUnencodableBodyFailsBeforeNetwork(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"error: a named byte-slice state (R59b)": {
			state:   blob(`{"a":1}`),
			want:    []string{"state: a plain []byte is ambiguous: engine.blob is a byte slice; send string(b) for text or RawJSON(b) for JSON"},
			isCause: codec.ErrPlainBytes,
		},
		"error: a named byte-slice extra member (R59b)": {
			state:   "x",
			extra:   []BodyMember{{"b", blob("hi")}},
			want:    []string{`extra body member "b": a plain []byte is ambiguous: engine.blob is a byte slice; send string(b)`},
			isCause: codec.ErrPlainBytes,
		},
		...
		"error: a pointer to a named byte slice extra member (NIT C)": {
			...
			want:    []string{`extra body member "p": a plain []byte is ambiguous: *engine.blob points to a byte slice; send string(b)`},
			isCause: codec.ErrPlainBytes,
		},
		"error: a nil pointer to a byte slice extra member (NIT C)": {
			state:   "x",
			extra:   []BodyMember{{"p", (*blob)(nil)}},
			want:    []string{`extra body member "p": nil *engine.blob holds no value, not a JSON value`},
...
func TestScalarStatesRefused(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"error: a named float":              {state: celsius(20), want: "engine.celsius encodes as a number"},
		...
		"error: a nil pointer":              {state: (*ticketState)(nil), want: "*engine.ticketState encodes as null"},
