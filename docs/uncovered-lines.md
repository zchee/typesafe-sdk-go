...
| `internal/codec/decode.go` | `(*DecodeError).Error` | `reason = "syntax error at byte " + strconv.Itoa(syntax.Pos) + ": " + syntax.Message()` | 1 | Defensive: only the lazy pass returns an `ast.SyntaxError`, and it re-reads only a body `traverse` has accepted: a malformed body fails in `traverse` and never reaches it (K36; `FuzzDecodeResponse` holds the one-scan verdict to the whole-body one on any input). `FuzzDecodePaths` (W6.1) checks that sonic's two parsers agree on valid bodies, the only kind it generates. |
...
| `internal/codec/decode.go` | `(*decoder).finish` | `return skipped, err` | 1 | Defensive: the lazy pass re-reads only a body `traverse` has accepted (a malformed body fails in `traverse`, K36), so it does not fail; `FuzzDecodePaths` (W6.1) checks that sonic's two parsers agree on valid bodies, the only kind it generates. |
...
| `internal/codec/lazy.go` | `(*decoder).lazy` | `return jsonErr(err)` | 6 | Defensive: the lazy pass re-reads, with sonic's ast, only a body `traverse` has accepted (a malformed body fails in `traverse`, K36), so sonic does not fail on it; `FuzzDecodePaths` (W6.1) checks that the two parsers agree on valid bodies, the only kind it generates. |
